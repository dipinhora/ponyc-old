...
ifeq ($(nightly),true)
  deploy_version := nightly-$(shell date +%Y%m%d)
endif
archive = ponyc-$(deploy_version).tar
package = $(PONY_BUILD_DIR)/ponyc-$(package_version)
...
deploy: test
	...
	$(SILENT)fpm -s dir -t deb -C $(package) -p $(PONY_BUILD_DIR)/bin --name ponyc --version $(deploy_version) --description "The Pony Compiler"
	$(SILENT)fpm -s dir -t rpm -C $(package) -p $(PONY_BUILD_DIR)/bin --name ponyc --version $(deploy_version) --description "The Pony Compiler"
	$(SILENT)git archive $(package_version) > $(PONY_BUILD_DIR)/bin/$(archive)
	$(SILENT)cp -r $(package)/usr/lib/pony/$(package_version)/stdlib-docs stdlib-docs
	$(SILENT)echo $(package_version) > .git_tag
	$(SILENT)tar rvf $(PONY_BUILD_DIR)/bin/$(archive) .git_tag
	$(SILENT)tar rvf $(PONY_BUILD_DIR)/bin/$(archive) stdlib-docs
	$(SILENT)bzip2 $(PONY_BUILD_DIR)/bin/$(archive)
	$(SILENT)rm -rf $(package) $(PONY_BUILD_DIR)/bin/$(archive) stdlib-docs .git_tag
